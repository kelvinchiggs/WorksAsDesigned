...
.DESCRIPTION
    ...
    Authentication:
    - Interactive authentication (default): Browser-based sign-in
    - App-only authentication: Use -TenantId, -ClientId, and -ClientSecret for automated scenarios
    
    Note: Some compliance features (DLP, Retention, Defender for Office 365) require manual
    verification in Microsoft Purview Compliance Portal as they are not accessible via Graph API.
    
.PARAMETER TenantId
    The Microsoft 365 Tenant ID (required for app-only authentication, optional for interactive)
    
.PARAMETER ClientId
    The Application (Client) ID for app-only authentication
    
.PARAMETER ClientSecret
    The Client Secret for app-only authentication (use with ClientId)
...
.EXAMPLE
    ...
    Runs complete audit with interactive authentication
    
.EXAMPLE
    .\Invoke-TeamsComprehensiveAudit.ps1 -TenantId "contoso.onmicrosoft.com" -ClientId "app-id" -ClientSecret "secret"
    Runs audit using app-only authentication with client credentials
...
.NOTES
    ...
    Modules     : Microsoft.Graph, MicrosoftTeams, ImportExcel
    ...
    1. Interactive (default): .\Invoke-TeamsComprehensiveAudit.ps1
    2. App-only: .\Invoke-TeamsComprehensiveAudit.ps1 -TenantId <id> -ClientId <id> -ClientSecret <secret>
#>
...
function Connect-RequiredServices {
    <#
    .SYNOPSIS
        Connects to Microsoft Graph and Teams using interactive or app-only authentication
    #>
    [CmdletBinding()]
    param(
        [Parameter(Mandatory = $false)]
    ...
    try {
        ...
        $useAppOnly = (-not [string]::IsNullOrEmpty($ClientId)) -and (-not [string]::IsNullOrEmpty($ClientSecret))
        
        if ($useAppOnly) {
            Write-Log "Using app-only authentication with client credentials" -Level INFO
            
            if ([string]::IsNullOrEmpty($TenantId)) {
                Write-Log "TenantId is required for app-only authentication" -Level ERROR
                return $false
            }
            
            # Connect to Microsoft Graph using client credentials
            Write-Log "Connecting to Microsoft Graph with app-only authentication..." -Level INFO
            ...
            # Connect to Microsoft Teams using access token
            Write-Log "Connecting to Microsoft Teams with app-only authentication..." -Level INFO
            
            # Get access token for Teams
            $token = Get-MgContext | Select-Object -ExpandProperty Scopes
            
            # Connect to Teams - app-only authentication
            try {
                ...
                Write-Log "Unable to connect to Teams with app-only auth, some features may be limited: $_" -Level WARNING
            }
            
        } else {
            Write-Log "Using interactive authentication" -Level INFO
            
            # Connect to Microsoft Graph with interactive auth
...
function Get-SecurityComplianceAudit {
    ...
    try {
        # 4.1 Threat Protection - Note: Requires Security & Compliance Center connection
        Write-Progress -Activity "Security & Compliance Audit" -Status "Checking threat protection settings..." -PercentComplete 10
        
        Write-Log "Defender for Office 365 checks require Security & Compliance Center PowerShell module" -Level INFO
        
        Add-Finding -Category 'Security & Compliance' -SubCategory 'Threat Protection' `
            -Finding 'Defender for Office 365 assessment requires manual verification' `
            -CurrentState "Cannot verify Defender for Office 365 settings via this script" `
            -RecommendedState "Defender for Office 365 Plan 2 with Teams protection enabled" `
            -BestPractice "Enable Safe Links, Safe Attachments, and Anti-phishing for Teams" `
            -RAGStatus 'Amber' `
            -Impact 'High' -Likelihood 'Medium' `
            -Remediation 'Manually verify Defender for Office 365 configuration in Security & Compliance Center' `
            -Timeline 'Quick Win (30 days)'
        
        # 4.2 Data Protection - Check for sensitivity labels via Graph (if available)
        Write-Progress -Activity "Security & Compliance Audit" -Status "Checking sensitivity labels..." -PercentComplete 30
        
        try {
            # Try to get information labels via Graph
            $uri = "https://graph.microsoft.com/v1.0/security/informationProtection/labelPolicySettings"
            $labelSettings = Invoke-MgGraphRequest -Uri $uri -Method GET -ErrorAction SilentlyContinue
            
            if ($labelSettings) {
                Write-Log "Sensitivity label configuration detected" -Level INFO
                $results['SensitivityLabelsConfigured'] = $true
                
                Add-Finding -Category 'Security & Compliance' -SubCategory 'Data Protection' `
                    -Finding 'Sensitivity labels configured' `
                    -CurrentState "Sensitivity labels policy detected" `
                    -RecommendedState "Labels deployed with auto-labeling and published to Teams" `
                    -BestPractice "Deploy sensitivity labels for Teams, SharePoint, and Office apps" `
                    -RAGStatus 'Green' `
                    -Impact 'Medium' -Likelihood 'Low' `
                    -Remediation 'Verify label policies cover Teams and enable auto-labeling' `
                    -Timeline 'Quick Win (30 days)'
            }
        } catch {
            Write-Log "Unable to retrieve sensitivity label information via Graph API: $_" -Level WARNING
            
            Add-Finding -Category 'Security & Compliance' -SubCategory 'Data Protection' `
                -Finding 'Sensitivity label status unknown' `
                -CurrentState "Cannot verify sensitivity label configuration" `
                -RecommendedState "Minimum 3-5 sensitivity labels with auto-labeling" `
                -BestPractice "Deploy sensitivity labels for Teams, SharePoint, and Office apps" `
                -RAGStatus 'Amber' `
                -Impact 'High' -Likelihood 'High' `
                -Remediation 'Manually verify sensitivity labels in Purview Compliance Portal' `
                -Timeline 'Medium Term (90 days)'
        }
        
        # 4.3 DLP Policies - Note: Requires Security & Compliance PowerShell
        Write-Progress -Activity "Security & Compliance Audit" -Status "Checking DLP policies..." -PercentComplete 50
        
        Write-Log "DLP policy checks require Security & Compliance Center PowerShell module" -Level INFO
        
        Add-Finding -Category 'Security & Compliance' -SubCategory 'Data Protection' `
            -Finding 'DLP policy assessment requires manual verification' `
            -CurrentState "Cannot verify DLP policies via this script" `
            -RecommendedState "Minimum 2-3 DLP policies for sensitive data (PII, financial, health)" `
            -BestPractice "Implement DLP policies for Teams chat and channels to prevent data leakage" `
            -RAGStatus 'Amber' `
            -Impact 'Critical' -Likelihood 'High' `
            -Remediation 'Manually verify DLP policies in Purview Compliance Portal covering Teams workload' `
            -Timeline 'Quick Win (30 days)'
        
        # 4.4 Retention Policies - Note: Requires Security & Compliance PowerShell
        Write-Progress -Activity "Security & Compliance Audit" -Status "Checking retention policies..." -PercentComplete 70
        
        Write-Log "Retention policy checks require Security & Compliance Center PowerShell module" -Level INFO
        
        Add-Finding -Category 'Security & Compliance' -SubCategory 'Data Protection' `
            -Finding 'Retention policy assessment requires manual verification' `
            -CurrentState "Cannot verify retention policies via this script" `
            ...
            -RAGStatus 'Amber' `
            -Impact 'High' -Likelihood 'Medium' `
            -Remediation 'Manually verify retention policies in Purview Compliance Portal' `
            -Timeline 'Medium Term (90 days)'
        
        # 4.5 Audit Logging - Check via Graph if possible
        Write-Progress -Activity "Security & Compliance Audit" -Status "Checking audit logging..." -PercentComplete 90
        
        try {
            # Try to check audit log configuration via Graph
            $uri = "https://graph.microsoft.com/v1.0/organization"
            $org = Invoke-MgGraphRequest -Uri $uri -Method GET -ErrorAction SilentlyContinue
            
            # Note: Unified Audit Log status not directly available via Graph API
            Write-Log "Audit logging status should be verified in Purview Compliance Portal" -Level INFO
            
            Add-Finding -Category 'Security & Compliance' -SubCategory 'Audit & Logging' `
                -Finding 'Audit logging requires manual verification' `
                -CurrentState "Cannot verify Unified Audit Log status via this script" `
                ...
                -RAGStatus 'Amber' `
                -Impact 'Critical' -Likelihood 'High' `
                -Remediation 'Verify Unified Audit Log is enabled in Purview Compliance Portal' `
                -Timeline 'Quick Win (30 days)'
...
function Get-SharePointOneDriveAudit {
    ...
    try {
        # 9.1 External Sharing Settings - Check via Graph API
        Write-Progress -Activity "SharePoint & OneDrive Audit" -Status "Checking sharing settings..." -PercentComplete 20
        
        try {
            # Get SharePoint settings via Graph
            $uri = "https://graph.microsoft.com/v1.0/admin/sharepoint/settings"
            $spoSettings = Invoke-MgGraphRequest -Uri $uri -Method GET -ErrorAction SilentlyContinue
            
            if ($spoSettings) {
                $sharingCapability = $spoSettings.sharingCapability
                $results['SharePointSharingCapability'] = $sharingCapability
                
                Write-Log "SharePoint sharing capability: $sharingCapability" -Level INFO
                ...
                if ($sharingCapability -eq 'externalUserAndGuestSharing') {
                    ...
                    Add-Finding -Category 'SharePoint & OneDrive' -SubCategory 'External Sharing' `
                        -Finding 'External sharing configured' `
                        -CurrentState "SharePoint sharing capability: $sharingCapability" `
                        -RecommendedState "Verify alignment with organizational security policy" `
                        -BestPractice "Regularly review and adjust sharing settings based on security requirements" `
                        -RAGStatus 'Amber' `
                        -Impact 'Medium' -Likelihood 'Medium' `
                        -Remediation 'Review sharing settings against organizational policy' `
                        -Timeline 'Medium Term (90 days)'
                }
            }
        } catch {
            Write-Log "Unable to retrieve SharePoint settings via Graph API: $_" -Level WARNING
            
            Add-Finding -Category 'SharePoint & OneDrive' -SubCategory 'External Sharing' `
                -Finding 'SharePoint settings require manual verification' `
                -CurrentState "Cannot verify SharePoint sharing settings via this script" `
                -RecommendedState "Anyone links disabled, limit to authenticated users" `
                -BestPractice "Configure appropriate sharing boundaries for Teams-connected sites" `
                -RAGStatus 'Amber' `
                -Impact 'High' -Likelihood 'Medium' `
                -Remediation 'Manually verify SharePoint sharing settings in SharePoint admin center' `
                -Timeline 'Quick Win (30 days)'
        }
        ...
        # Versioning is site-level - provide general guidance
        ...
        # 9.3 Storage Governance
        Write-Progress -Activity "SharePoint & OneDrive Audit" -Status "Checking storage settings..." -PercentComplete 80
        
        Add-Finding -Category 'SharePoint & OneDrive' -SubCategory 'Storage Governance' `
            -Finding 'Storage governance review recommended' `
            -CurrentState "Default storage quotas may be in place" `
            -RecommendedState "Appropriate storage quotas and monitoring for Teams sites" `
            -BestPractice "Implement storage quotas and monitoring to prevent uncontrolled growth" `
            -RAGStatus 'Amber' `
            -Impact 'Low' -Likelihood 'Medium' `
            -Remediation 'Review and configure appropriate storage limits for Teams-connected sites' `
...
function Invoke-MainAudit {
    ...
    if (-not (Connect-RequiredServices @connectParams)) {
...
# Execute main audit
Invoke-MainAudit -TenantId $TenantId -ClientId $ClientId -ClientSecret $ClientSecret -IncludeVoice:$IncludeVoice -IncludeDevices:$IncludeDevices -OutputPath $OutputPath
