...
function Get-TeamsAppManagementAuditData {
    <#
    .SYNOPSIS
        Collects app permission policies and setup policies.
        NOTE: App catalog enumeration has been REMOVED per requirements.
    #>
    [CmdletBinding()] param()
    Write-AuditLog "  Collecting app management data (catalog enumeration excluded)..." -Level Info
    ...
    try {
        ...
        if ($permPolicies) { $result.PermissionPolicies = @($permPolicies) }
        # App setup policies
...
function New-TeamsAuditExcelReport {
    ...
    try {
        ...
        # ── Assess each configuration domain ──
        ...
        if ($AuditData.TeamsMeetings -and $AuditData.TeamsMeetings.MeetingPolicies) {
            $allFindings += @(Get-PolicyDomainFindings -DomainName "Meeting Policies" -Policies $AuditData.TeamsMeetings.MeetingPolicies -BPRef $BestPractices.MeetingPolicies)
        }
        ...
        if ($AuditData.TeamsMessaging -and $AuditData.TeamsMessaging.MessagingPolicies) {
            $allFindings += @(Get-PolicyDomainFindings -DomainName "Messaging Policies" -Policies $AuditData.TeamsMessaging.MessagingPolicies -BPRef $BestPractices.MessagingPolicies)
        }
        ...
        if ($AuditData.TeamsGovernance -and $AuditData.TeamsGovernance.ExternalAccess -and $AuditData.TeamsGovernance.ExternalAccess.Count -gt 0) {
            $allFindings += @(Get-PolicyDomainFindings -DomainName "External Access" -Policies $AuditData.TeamsGovernance.ExternalAccess -BPRef $BestPractices.ExternalAccess -IsSinglePolicy)
        }
        ...
        if ($AuditData.TeamsGovernance -and $AuditData.TeamsGovernance.GuestAccess -and $AuditData.TeamsGovernance.GuestAccess.Count -gt 0) {
            $allFindings += @(Get-PolicyDomainFindings -DomainName "Guest Access" -Policies $AuditData.TeamsGovernance.GuestAccess -BPRef $BestPractices.GuestAccess -IsSinglePolicy)
        }
        ...
        if ($AuditData.TeamsAppManagement -and $AuditData.TeamsAppManagement.SetupPolicies) {
            $allFindings += @(Get-PolicyDomainFindings -DomainName "App Policies" -Policies $AuditData.TeamsAppManagement.SetupPolicies -BPRef $BestPractices.AppPolicies)
        }
        ...
        if ($AuditData.TeamsUpdatePolicies -and $AuditData.TeamsUpdatePolicies.UpdatePolicies) {
            $allFindings += @(Get-PolicyDomainFindings -DomainName "Update Policies" -Policies $AuditData.TeamsUpdatePolicies.UpdatePolicies -BPRef $BestPractices.UpdatePolicies)
        }
        ...
        if ($AuditData.TeamsSettings -and $AuditData.TeamsSettings.ClientConfiguration -and $AuditData.TeamsSettings.ClientConfiguration.Count -gt 0) {
            $allFindings += @(Get-PolicyDomainFindings -DomainName "Teams Settings" -Policies $AuditData.TeamsSettings.ClientConfiguration -BPRef $BestPractices.TeamsSettings -IsSinglePolicy)
        }
        ...
        if ($AuditData.TeamsMeetings -and $AuditData.TeamsMeetings.LiveEventPolicies) {
            $allFindings += @(Get-PolicyDomainFindings -DomainName "Live Events" -Policies $AuditData.TeamsMeetings.LiveEventPolicies -BPRef $BestPractices.LiveEventsPolicies)
        }
        ...
        if ($AuditData.TeamsEncryption -and $AuditData.TeamsEncryption.EncryptionPolicies) {
            foreach ($policy in $AuditData.TeamsEncryption.EncryptionPolicies) {
                ...
                $allFindings += [PSCustomObject]@{
                    Domain="Enhanced Encryption"; PolicyName=[string]$policy.Identity; Setting="CallingEndToEndEncryptionEnabledType"
        ...
        # ══════════════════════════════════════════════════════════════════════
        # E911 COMPLIANCE ASSESSMENT (only when Teams Phone is enabled)
        # ══════════════════════════════════════════════════════════════════════
        Write-Progress -Activity "Assessing Configuration" -Status "E911 Compliance" -PercentComplete 70
        if ($AuditData.TeamsPhone -and $AuditData.TeamsPhone.TeamsPhoneEnabled) {
            Write-AuditLog "  Assessing E911 Compliance (Teams Phone is enabled)..." -Level Info
            $e911Checks = @(
                @{
                    Setting = "EmergencyCallingPolicyConfigured"
                    Value   = ($AuditData.TeamsPhone.EmergencyCallingPolicies.Count -gt 0)
                }
                @{
                    Setting = "EmergencyCallRoutingPolicyConfigured"
                    Value   = ($AuditData.TeamsPhone.EmergencyCallRoutingPolicies.Count -gt 0)
                }
                @{
                    Setting = "NetworkSitesConfigured"
                    Value   = ($AuditData.TeamsPhone.NetworkSites.Count -gt 0)
                }
                @{
                    Setting = "NotificationGroupConfigured"
                    Value   = (($AuditData.TeamsPhone.EmergencyCallingPolicies |
                                Where-Object { -not [string]::IsNullOrEmpty($_.NotificationGroup) }).Count -gt 0)
                }
            )
            foreach ($check in $e911Checks) {
                ...
                if ($bp) {
                    ...
                    $allFindings += [PSCustomObject]@{
                        Domain="E911 Compliance"; PolicyName="Org-Wide"; Setting=[string]$check.Setting
        ...
        else { Write-AuditLog "  E911 assessment skipped (Teams Phone not enabled)." -Level Info }
        ...
        # FIX #2: Conditional formatting applies colour fills to RAG indicator cells
        ...
        $summaryData | Export-Excel -Path $reportPath -WorksheetName "Executive Summary" `
            -AutoSize -TableName "ExecutiveSummary" -TableStyle Medium2 `
        ...
        if ($allFindings.Count -gt 0) {
            # Conditional formatting for RAG and Risk columns
            $ragAssessmentCF = @(
                New-ConditionalText -Text "Green" -BackgroundColor Green -ConditionalTextColor White
                New-ConditionalText -Text "Amber" -BackgroundColor Orange -ConditionalTextColor Black
                New-ConditionalText -Text "Red" -BackgroundColor Red -ConditionalTextColor White
                New-ConditionalText -Text "Critical" -BackgroundColor Red -ConditionalTextColor White
                New-ConditionalText -Text "High" -BackgroundColor OrangeRed -ConditionalTextColor White
                New-ConditionalText -Text "Medium" -BackgroundColor Orange -ConditionalTextColor Black
                New-ConditionalText -Text "Low" -BackgroundColor Green -ConditionalTextColor White
            )

            $allFindings |
                Select-Object Domain, PolicyName, Setting, CurrentValue, DefaultValue, BestPractice, `
                ...
                Export-Excel -Path $reportPath -WorksheetName "RAG Assessment" -AutoSize `
                    -TableName "RAGAssessment" -TableStyle Medium2 -Append -ConditionalText $ragAssessmentCF
        }
        ...
        $tierSelect = @(
            ...
            'Domain','PolicyName','Setting','RAGStatus','RiskLevel',
        ...
        if ($roadmap.QuickWins.Count -gt 0) {
            $roadmap.QuickWins | Select-Object $tierSelect | Sort-Object Priority -Descending |
                Export-Excel -Path $reportPath -WorksheetName "Quick Wins (30 Days)" -AutoSize `
                    -TableName "QuickWins" -TableStyle Medium2 -Append -ConditionalText $tierCF
        }
        if ($roadmap.MediumTerm.Count -gt 0) {
            $roadmap.MediumTerm | Select-Object $tierSelect | Sort-Object Priority -Descending |
                Export-Excel -Path $reportPath -WorksheetName "Medium Term (90 Days)" -AutoSize `
                    -TableName "MediumTerm" -TableStyle Medium2 -Append -ConditionalText $tierCF
        }
        if ($roadmap.Strategic.Count -gt 0) {
            $roadmap.Strategic | Select-Object $tierSelect | Sort-Object Priority -Descending |
                Export-Excel -Path $reportPath -WorksheetName "Strategic (6-12 Months)" -AutoSize `
                    -TableName "Strategic" -TableStyle Medium2 -Append -ConditionalText $tierCF
        }
        ...
        if ($allFindings.Count -gt 0) {
            ...
            if ($riskMatrix) {
                $riskMatrix | Export-Excel -Path $reportPath -WorksheetName "Risk Matrix" -AutoSize `
                    -TableName "RiskMatrix" -TableStyle Medium2 -Append -ConditionalText $riskMatrixCF
            }
        }
        ...
        if ($AuditData.TeamsSecurity -and $AuditData.TeamsSecurity.PIMRoleAssignments -and $AuditData.TeamsSecurity.PIMRoleAssignments.Count -gt 0) {
            ...
            $pimCF = @(
                New-ConditionalText -Text "Permanent" -BackgroundColor Red -ConditionalTextColor White
                New-ConditionalText -Text "Eligible" -BackgroundColor Green -ConditionalTextColor White
                New-ConditionalText -Text "Red" -BackgroundColor Red -ConditionalTextColor White
                New-ConditionalText -Text "Green" -BackgroundColor Green -ConditionalTextColor White
            )
            ...
            $pimData | Export-Excel -Path $reportPath -WorksheetName "PIM Assignments" -AutoSize `
                -TableName "PIMAssignments" -TableStyle Medium2 -Append -ConditionalText $pimCF
        }
        ...
        # Return report path, findings, and roadmap for consumption by Word report generation
        return @{
...
function New-TeamsAuditWordReport {
    ...
    .DESCRIPTION
        Exports report data to JSON and generates a Word document using the PSWriteWord
        module if available, or exports structured data for external generation.
        The report includes:
        - Cover page with tenant and audit metadata
        - Executive summary with RAG distribution
        - Remediation roadmap by tier (Quick Wins, Medium Term, Strategic)
        - Detailed findings by domain with policy names
        - E911 compliance section (when Teams Phone is enabled)
        - PIM role assignment analysis with JIT recommendations
    ...
    Write-AuditLog "Generating Word report data..." -Level Header
    $timestamp = $script:StartTime.ToString("yyyyMMdd_HHmmss")
    ...
    try {
        # Prepare structured report data with all findings including policy names
        $reportData = @{
            metadata = @{
            ...
            findings = @($AllFindings | ForEach-Object {
                @{
                    ...
                    Setting = [string]$_.Setting
                    CurrentValue = [string]$_.CurrentValue
                    BestPractice = [string]$_.BestPractice
                    RAGStatus = [string]$_.RAGStatus
                    RiskLevel = [string]$_.RiskLevel
                    RiskScore = [int]$_.RiskScore
                    SecurityDomain = [string]$_.SecurityDomain
        ...
        # Export report data to JSON for Word generation
        $reportData | ConvertTo-Json -Depth 10 | Out-File $jsonPath -Encoding UTF8
        Write-AuditLog "  Report data exported: $jsonPath" -Level Success

        # Generate Word document using PSWriteWord (validated as prerequisite during module check)
        # CRITICAL: All PSWriteWord cmdlets return Xceed.Document.NET objects to the pipeline.
        # Use -Supress $true on Add-WordText/Save-WordDocument and | Out-Null on others
        # to prevent pipeline pollution that causes empty files and console spam.
        ...
        Add-WordText -WordDocument $doc -Text "Tenant: $($reportData.metadata.tenantName)" -HeadingType Heading2 -Supress $true
        ...
        foreach ($link in $reportData.appendix.officialLinks) {
            Add-WordText -WordDocument $doc -Text $link -Supress $true
        }

        Save-WordDocument $doc -FilePath $wordPath -Supress $true
